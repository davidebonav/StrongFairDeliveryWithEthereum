...
/*
    ...
    PROTOCOLLO2 : 
    A -> B : { emailFrom, emailTo, encMsg, label, timestamp1, H_name } 
    A -> emit ( H_name, H(emailFrom||emailTo||encMsg||timestamp1||label), label, A(msg.sender), currentTimstamp )
    
    B -> emit ( H_name, H(emailFrom||emailTo||encMsg||timestamp1||label+1), label, pubK, A(msg.sender), currentTimstamp )
